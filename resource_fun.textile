...
The same mechanism gets into play in the _onQuery_ on the _ballot_ resource. But an onQuery maps to an HTTP GET request, there's no body, hence the URL parameters start right from the first block variable. Also notice how we reply in both branches of the _if_. This is important, requests always need a response. In a block without a _reply_, a default empty reply gets generated, which isn't what we want here.
